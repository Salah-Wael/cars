...
class CarImagesController extends Controller
{
    ...
    public function deleteImage($imageId)
    {
        ...
        if ($carImage) {
            ImageController::deleteImage($carImage->image_path, 'assets/img/products/');
            $carId = $carImage->car_id;
            DB::table('car_images')->where('id', $imageId)->delete();
            return redirect()->route('car.show.images', compact('productId'))->with(['success' => "product's image deleted successfully"]);
        } else {
