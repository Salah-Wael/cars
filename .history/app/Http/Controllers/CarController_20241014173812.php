...
class CarController extends Controller
{
    ...
    {
        $cars = DB::table('cars')
        ->leftJoin('car_images', 'cars.id', '=', 'car_images.car_id') // Join cars with car_images table
        ->where('cars.post_status', '=', 0) // Filter by post_status
        ->select('cars.*', DB::raw('GROUP_CONCAT(car_images.image) as images')) // Select all car columns and concatenate the images
        // ->groupBy('cars.id') // Group by car ID to avoid duplication
