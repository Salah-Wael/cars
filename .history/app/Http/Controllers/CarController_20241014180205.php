...
class CarController extends Controller
{
    ...
    public function carsRequests()
    {
        ...
        ->where('cars.post_status', '=', 0)
        ->select('cars.id', 'cars.type', 'cars.car_model', 'cars.color', 'cars.plate', 'cars.status', 'cars.price', 'cars.description', DB::raw('GROUP_CONCAT(car_images.image) as images'))
        ->groupBy('cars.id', 'cars.type', 'cars.car_model', 'cars.color', 'cars.plate', 'cars.status', 'cars.price', 'cars.description')
        ->get();

        // Convert each car's concatenated images string into an array
        foreach ($cars as $car) {
            $car->images = $car->images ? explode(',', $car->images) : []; // Ensure it's an array even if empty
        }
