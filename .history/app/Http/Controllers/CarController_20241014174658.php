...
class CarController extends Controller
{
    ...
    public function carsRequests()
    {
        ...
        ->groupBy('cars.id', 'cars.type', 'cars.car_model', 'cars.color', 'cars.plate', 'cars.status', 'cars.price', 'cars.description') // Group by all selected car columns
        ->get();

        foreach ($cars as $car) {
            // Explode the images for each car
            $car->images = explode(',', $car->images);
        }


        return view('car.cars-table', compact('cars', 'carImages'));
    }
