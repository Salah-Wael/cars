...
class CarController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        ->where('cars.id', $id) // Filter by car ID
            ...
            ->select('cars.id', 'cars.type', 'cars.car_model', 'cars.plate', 'cars.image', 'cars.description', 'cars.color', 'cars.price', 'cars.status', DB::raw('GROUP_CONCAT(car_images.image) as images')) // Select fields
            ->groupBy('cars.id', 'cars.type', 'cars.car_model', 'cars.plate', 'cars.image', 'cars.description', 'cars.color', 'cars.price', 'cars.status') // Group by all selected car fields
        ->first(); // Retrieve the car with grouped images
        $carImages = explode(',', $car->images);

        if ($car) {
            $carColors = CarColor::select('color_name as color')->get();
            $carTypes = CarManufacturer::orderBy('manufacturer')->select('manufacturer as type')->get();
            $carStatus = $this->carStatus;
    ...
    public function update(Request $request, $id)
    {
        ...
        // Step 2: Retrieve the car and its images
        $car = Car::with('images', 'user')->find($id);

        if ($car) {
            // Step 3: Update car properties from the request
            ...
            // Step 4: Handle image upload if files are provided
            if ($request->hasFile('image')) {
                // Detach old images (one-to-many) or sync new images (if it’s a many-to-many relationship)
                $car->images()->delete(); // Use delete() for one-to-many

                // Store new images using your ImageController logic
                $imagesArray = ImageController::storeImages($request, 'image', 'assets/img/cars/');

                // Sync the new image paths or attach them if you're handling images as a related model
                foreach ($imagesArray as $imagePath) {
                    // You can add image to pivot table or directly update the field, depending on your relationship
                    $car->images()->create([
                        'image' => $imagePath
                    ]);
                }

                // Assuming you want to store the first image as the car's main image
                $car->image = $imagesArray[0];

                // Update timestamps with touch
                $car->touch();
            }

            // Step 5: Check if car model has dirty attributes and update updated_at manually
            if ($car->isDirty()) {
                $car->updated_at = now();
            }

            // Step 6: Role-based post status (setting `post_status` based on role)
            if (Auth::user()->role == 'user') {
                $car->post_status = 0;
            }

            // Step 7: Save the updated car model
            $car->save();
        ...
        // If no car found, show an error page
        return view('error.404')->with(['message' => 'Car ID not found']);
    }
    ...
    //     // Find car by ID, abort with 404 if not found
    //     // $car = Car::with('images')->findOrFail($id);
    //     $car = Car::join('car_images as ci', 'cars.id', '=', 'ci.car_id')
    //     ->select('cars.*', 'ci.image')
    //     ->findOrFail($id);

    //     // Update the car fields with validated data
    //     $car->fill([
    ...
    //     // Handle images
    //     if ($request->hasFile('image')) {
    //         // Detach existing images and store new ones
    //         $car->images()->detach();
    //         $imagesArray = ImageController::storeImages($request, 'image', 'assets/img/cars/');

    //         // Sync new images and set the first one as the car's main image
    //         $car->images()->sync($imagesArray);
    //         $car->image = $imagesArray[0]; // Assuming the first image is the main one
    //     }

    //     // If the model or the images have been changed, update the timestamps and reset post status
    //     if ($car->isDirty() || $request->hasFile('image')) {
    //         $car->updated_at = now();
    //         $car->post_status = 0; // Reset post status if changes are made
    //     }

    //     // Save the car with the updates
    //     $car->save();
