...
Route::controller(CarImagesController::class)->name('car.')->group(function () {
    Route::get('/product/{productId}/images', 'showProductImages')->name('show.images')->middleware('auth');
    Route::post('/product/{productId}/store/images', 'storeImages')->name('store.images')->middleware('checkrole:admin');
    Route::delete('/product/image/{imageId}/delete', 'deleteImage')->name('delete.image')->middleware('checkrole:admin');
});
