...
<div class="container">
    ...
    @method('PUT') <!-- Specify the PUT method for updating -->
        ...
        <div class="mb-3">
            ...
            <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" required>
                ...
                @foreach($carTypes as $carType)
                    <option value="{{ $carType->type }}" {{ $car->type == $carType->type ? 'selected' : '' }}>
                        {{ $carType->type }}
                    </option>
                @endforeach
                <option value="Other" {{ $car->type == 'Other' ? 'selected' : '' }}>
                    Other
                </option>
            </select>
            @error('type')
        ...
        <div class="mb-3">
            ...
            <select class="form-select @error('color') is-invalid @enderror" id="color" name="color" required>
                ...
                @foreach ($carColors as $carColor)
                    <option value="{{ $carColor->color }}" {{ $car->color == $carColor->color ? 'selected' : '' }}>
                        {{ $carColor->color }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <div class="mb-3">
                <p>Current Images:</p>
                @forea($carImages as $image)
                @dump($image)
                    <div style="display: inline-block; margin-right: 10px;">
                        <img src="{{ asset('assets/img/cars/' . ($image ?? $car->image)) }}" alt="Car Image" style="width: 100px; height: 100px;">
                    </div>
                @empty
                    <div style="display: inline-block; margin-right: 10px;">
                        <img src="{{ asset('assets/img/cars/' . $car->image) }}" alt="Car Image" style="width: 100px; height: 100px;">
                    </div>
                @endforelse
            </div>
