...
<div class="container">
    ...
    <div class="mb-3">
        ...
        <select class="form-select @error('color_name') is-invalid @enderror" id="color_name" name="color_name" required>
            <option value="">Select a color</option> <!-- Placeholder option -->
            @foreach ($carColors as $carColor)
                <option value="{{ $carColor->color_name }}" {{ old('color_name') == $carColor->color_name ? 'selected' : '' }}>
                    {{ $carColor->color_name }}
                </option>
            @endforeach
        </select>
