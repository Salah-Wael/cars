...
<div class="container">
    ...
    <div class="mb-3">
        ...
        <select class="form-control @error('car_type') is-invalid @enderror" id="car_type" name="car_type" required>
            ...
            @foreach($carTypes as $carType)
                <option value="{{ $carType->type }}" {{ old('car_type') == $carType->type ? 'selected' : '' }}>
                    {{ $carType->type }}
                </option>
                @endforeach
            <option value="{{ Other }}" {{ old('car_type') == $carType->type ? 'selected' : '' }}>
                Other
            </option>
        </select>
        @error('car_type')
    ...
    <div class="mb-3">
        ...
        <select class="form-select @error('color') is-invalid @enderror" id="color" name="color_name" required>
            <option value="">Select a color</option> <!-- Placeholder option -->
            @foreach ($carColors as $carColor)
                <option value="{{ $carColor->name }}" {{ old('color_name') == $carColor->name ? 'selected' : '' }}>
                    {{ $carColor->name }}
                </option>
            @endforeach
        </select>
