                ...
                <div class="row">
                    @foreach($cars as $car)
                        <div class="new_car_car col-md-4">
                            <div class="car_card">
                                <img src="{{ asset('assets/img/cars/{{ $car->image }}') }}" alt="2024 Chevrolet Camaro ILT Convertible">
                                <div class="car_details">
                                    <h4><strong>Model:</strong> {{ $car->car_model }}</h4>
                                    <p><strong>Color:</strong> {{ $car->color }}</p>
                                    @if ($car->plate)
                                        <p><strong>Plate Number:</strong> {{ $car->plate }}</p>
                                    @endif
                                    <p><strong>Price:</strong> EGP {{ number_format($car->price)}}</p>
                                    <div class="car_status"><strong>Status:</strong>  {{ strtoupper($car->status) }}</div>
                                    @if ($car->description)
                                    {{  }}...
                                        <p><strong>Description:</strong> {{substr($new->content, 0, 30) $car->description }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
